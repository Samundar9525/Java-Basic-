(function () {
  ...
  function buildCategories() {
    const categories = ["All topics", ...new Set(state.all.map((item) => item.category))];
  ...
  function loadPool() {
    ...
    if (selected === "All topics") {
      ...
      state.pool = state.all.filter((item) => item.category === selected);
    }
    state.index = 0;
    state.viewed = 0;
    renderQuestion();
  ...
  function setMood(mood) {
    els.stage.classList.remove("is-asking", "is-answering", "is-thinking", "is-correct", "is-wrong");
  ...
  function renderQuestion() {
    ...
    if (!item) {
      ...
      els.questionText.textContent = "No questions available for this topic.";
      els.answerText.textContent = "Try another topic from the filter.";
      els.answerBubble.classList.remove("is-hidden", "is-typing");
    ...
    stopTyping();
    state.viewed = Math.max(state.viewed, state.index + 1);
    state.currentQuestion = item.question;
    state.fullAnswer = `Q: ${item.question}\n\nA: ${item.options[item.answer]}\n\n${item.answerText}`;
    setMood("asking");
    els.category.textContent = item.category;
    els.counter.textContent = `Question ${state.index + 1} of ${state.pool.length}`;
  ...
  function updateStats() {
    els.score.textContent = state.pool.length ? state.index + 1 : 0;
  ...
  function typeAnswer() {
    ...
    state.typingTimer = setInterval(() => {
      ...
      els.answerText.textContent = state.fullAnswer.slice(0, cursor);
      els.answerText.scrollTop = els.answerText.scrollHeight;

      if (cursor >= state.fullAnswer.length) {
        stopTyping();
        els.answerText.textContent = state.fullAnswer;
        els.answerBubble.classList.remove("is-typing");
        els.answerBtn.disabled = false;
  ...
  els.answerBtn.addEventListener("click", typeAnswer);
  ...
  els.categoryFilter.addEventListener("change", loadPool);
